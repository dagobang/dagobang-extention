...
export const createNewCoinSniperTrade = (deps: {
  ...
  const resolveSignalChainIdLoose = (input: {
    tokenAddress?: string | null;
    signal?: UnifiedMarketSignal | null;
  }) => {
    const addr = normalizeAddressKey(input.tokenAddress);
    const tokens = Array.isArray(input.signal?.tokens) ? input.signal.tokens : [];
    if (addr) {
      const matched = tokens.find((x) => normalizeAddressKey((x as any)?.tokenAddress) === addr);
      const tokenChain = String((matched as any)?.chain || '').trim();
      if (tokenChain) {
        const chainId = getChainIdByName(tokenChain);
        if (Number.isFinite(chainId) && chainId > 0) return chainId;
      }
    }
    const signalChain = String(input.signal?.chain || '').trim();
    if (!signalChain) return null;
    const chainId = getChainIdByName(signalChain);
    return Number.isFinite(chainId) && chainId > 0 ? chainId : null;
  };
  const toModeMetaKey = (chainId: number, tokenAddrLower: string) => buildScopedTokenKey(chainId, tokenAddrLower);
  ...
  const broadcastMatchedSound = async (strategy: any, tokenAddress?: string) => {
  ...
  const getKey = (chainId: number, tokenAddress: string, opts?: { dry?: boolean; walletAddress?: string }) => {
    ...
    const walletKey = !dry ? normalizeWalletAddressKey(opts?.walletAddress) : 'all-wallets';
    return `${dry ? 'dry:' : ''}${toScopedTokenKey(chainId, tokenAddress)}:${walletKey}:new-coin`;
  };
  ...
  const shouldEmitBuyFailureRecord = (input: {
    ...
    const modeTag = currentStrategyMode === 'xmode_task'
      ...
      : 'auto_filter';
    const key = `${input.reason}:${input.chainId}:${normalizeAddressKey(input.tokenAddress)}:${signalStableId}:${modeTag}`;
    const ttlMs =
  ...
  const emitRecord = (record: any) => {
    ...
    const tokenAddrLower = normalizeAddressKey(record?.tokenAddress);
    const resolvedChainId = (() => {
      const fromSignal = resolveSignalChainIdLoose({
    ...
    const modeMetaKey = tokenAddrLower ? toModeMetaKey(resolvedChainId, tokenAddrLower) : '';
    const fallbackModeMeta = (() => {
      ...
      if (record?.side === 'sell' && tokenAddrLower) {
        return latestModeMetaByToken.get(modeMetaKey) ?? { strategyMode: 'auto_filter' as const };
      }
    ...
    if (modeMetaKey && resolvedModeMeta.strategyMode) {
      latestModeMetaByToken.set(modeMetaKey, resolvedModeMeta);
    }
  ...
  const getWsAthMarketCapUsd = (chainId: number, tokenAddress: string): number => {
    const snapshots = wsSnapshotsByAddr.get(toScopedTokenKey(chainId, tokenAddress)) ?? [];
    let ath = 0;
  ...
  const ensureAutoTasksFromSignal = async (input: {
    ...
    for (const token of tokens) {
      ...
      const chainId = resolveTradeChainId({
        tokenChain: metrics.chain,
        signalChain: input.signal.chain,
        fallbackChainId: input.settings.chainId,
        settings: input.settings,
      });
      pushWsSnapshot(chainId, metrics.tokenAddress, metrics);
      ...
      const athMcapUsd = getWsAthMarketCapUsd(chainId, metrics.tokenAddress);
  ...
  const pickCandidates = (signal: UnifiedMarketSignal, strategy: any) => {
    ...
    for (const t of tokens) {
      const addr = normalizeAddressKey(t.tokenAddress);
    ...
    const candidates = unique
      .map((t) => {
        ...
        if (m?.tokenAddress) {
          ...
          pushWsSnapshot(chainId, m.tokenAddress, m);
        }
        return { t, m };
      })
      .filter((x) => {
        ...
        const chainId = resolveTradeChainId({
          tokenChain: x.m?.chain,
          signalChain: signal.chain,
          fallbackChainId: 56,
          settings: defaultSettings,
        });
        const confirm = computeWsConfirm(chainId, tokenAddress, now, strategy);
  ...
  const pickTaskCandidates = (signal: UnifiedMarketSignal, strategy: any, task: NewCoinXmodeSnipeTask) => {
    ...
    const candidates = unique
      ...
      .filter((x) => {
        ...
        const confirm = computeWsConfirm(chainId, tokenAddress, now, strategyForTask);
        if (!confirm.pass) return false;
        return true;
      });
    candidates.sort((a, b) => {
  ...
  const executeXmodeTaskBuys = async (input: {
    ...
    chainId: number;
    settings: any;
    onMatched?: (tokenAddress?: string) => Promise<void> | void;
  }) => {
  ...
  const handleMarketSignal = async (signal: UnifiedMarketSignal) => {
    try {
      ...
      if (effectiveTaskModeEnabled) {
        ...
        const playMatchedSoundOnce = async (tokenAddress?: string) => {
