...
export function XSniperFilterSection({
  ...
  const togglePlatform = (value: string) => {
    ...
    updateTwitterSnipe({ platforms: Array.from(set) } as any);
  };
  const toggleAllPlatforms = () => {
    if (!platformOptions.length) return;
    if (allSelected) {
      updateTwitterSnipe({ platforms: [] } as any);
      return;
    }
    updateTwitterSnipe({ platforms: platformOptions.map((x) => x.value) } as any);
  };
  return (
                    ...
                    {platformOptions.map((item) => {
                      ...
                      return (
                        <button
                          key={item.value}
                          type="button"
                          className={`rounded-full border px-2.5 py-1 text-[12px] transition ${active
